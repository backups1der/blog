+++
title = "baba sayfa 🙃"
description = "cehennemin kapısı"
insert_anchor_links = "left"
[extra]
no_header = true
+++

<div class="container-fill">
  <div>

  <!-- {{ image(url="/assets/parappa.png", alt="Just my avatar", end=true, no_hover=true) }} -->
  <!-- <figcaption>replace this image lel</figcaption>   -->
  <div id="title">

  # merhaba, ben kornaman <span class="rainbow">( {{ sticker(path="/parappa.png", name="ben, kesinlikle", big=true, classes="pfp") }} )</span>

  </div>

  internetteki küçük köşeme hoş geldin, burada aptal düşüncelerimi, fikirlerimi ve projelerimi paylaş<span class="spoiler">am</span>ıyorum. <small>(o sürprizbozanlı kısım çok sapık duracak FDSADFHJGFDFHJHGF 😭)</small>

  buradaki şeyleri yazan ve birkaç özellik ekleyen kişi benim (waycanınaa). kesinlikle bu temayı kullanan arkadaşlarımın bloglarından kodlar çalmadım(!)

  <div class="buttons start big">
    <a href="#welcome" class="suggested">hoşgeldin ↓</a>
    <a href="/tr/about#right" class="">hakkımda -></a>
  </div>
  </div>
</div>

## <span class="rainbow">blo(c)k</span>'a hoşgeldin! {#welcome}

evet, bu aptal blog için arkadaşlarımdan isim istedim ve en iyi ve havalı isim buydu <small>evet evet, en iyi isim basbayağı *blok* idi</small>.

siteyi biraz incelediyseniz bu sitenin bir de ingilizce çevirisini görebilirsiniz, bu yüzden minik bir değişiklik yaparak bu ismi aldım.

## daha neyi geliştiriyor olabilirsin aw-
daha bir sürü şeyi. mesela:
- özel kısa kodlar ve stiller
- daha iyi (ve havalı) ana sayfa
- daha fazla içerik ve özellikler
- daha iyi (ve havalı) bir blog deneyimi <small>(ne, aynı cümleyi yine mi kullandım? bunu görmezden gelip alay mı ediyormuşum bir de? <del>sanane olm</del>)</small>

şaka bir yana, bu siteyi geliştirirken amacım neydi, biliyor musunuz?
<details>
  <summary>geliştirme hedeflerim</summary>
  <ul>
    <li>kafa dağıtmak</li>
    <li>rahat ve özgürce konuşup bilgi paylaşabileceğim bir yer oluşturmak</li>
    <li>deneyim kazanmak</li>
    <li>hasan hocadan kaçmak 😀</li>
  </ul>  
</details>

<details>
  <summary> boş beleş şeyler 4k uhd izle indir</summary>

  # büyük teşekkürler 
  -> [daudix](https://daudix.one)'e [bu yazdığı tema](https://ametrine.daudix.one) için teşekkürlerimi iletmek istiyorum. gerçekten, onun blog'una bir göz atın en azından; gerçekten mükkemmel yazıları var! <small>(yani en azından benimkikadar berbat değil :D)</small>

  -> [mambuco](https://mambuco.dev#down)'ya da havalı birisi olduğu için VE (direkt olarak olmasa da, github reposunu açık kaynaklı bulundurması bile yetti bana) bana yardım ettiği için (<u>ve</u> (*uh*) bunun dışında havalı birisi olduğu için) teşekkürler

  -> [(ve diğer) <span class="snug">snug nook</span>('erlara)](https://snugnook.org) üstünde dayanabileceğim büyük mental dayanak oldukları için teşekkürlerimi iletiyorum. yani uh onların arasında o kadar da bok çukuruna dönmüş mental sağlığımdan bahsetmemiş olabilirim ama *hey*, BU İNSANLAR ÇOK <span class="rainbow">**HAVALI**</span>LAR XD

  ---

  -> [sparrow99tr_](https://sparrow99.me/)'a da teşekkür ederim, çalışmayan beynimin yerine fikirlerimi onaylayan eleman bu idi. ayrıca o "*orta boyutlu youtuberlar*" kategorisine giriyor, bunu değiştirip "*xl boyutlu youtuberlar*" kategorisine şutlamak için [kanalına abone olun](https://youtube.com/@sparrow99_?sub_confirmation=1) <small>([hayır bunun için bir rüşvet almadım](silahzoru.png). hadi be, yapın bunu bir zahmet-)</small>

  -> bana bu bloğun ismini öneren arkadaşa da teşekkürle- <small>(böyle birisi yok bu arada,sınıftaki kişilerin de çaktığı gibi *çoklu kişilik bozukluğu*nun sonuçlarını görüyorsunuz FDSFGHJKHGFDGHJK)</small>

  <span class="spoiler">-> [parappa the rapper](https://en.wikipedia.org/wiki/PaRappa_the_Rapper)a da gerçek olup tatlı kaldığı için teşekkürler. (ve uh başlıktaki resim parappa'nın bir *fanartı*ydı haha)  
  <small>yemin ederim ki bu yapay zekalar *bu kadar* hızlı öğrenebiliyor, bu son teşekkür onun önerisiydi DSFDGHJHGFDSGHJKGFD :sob:</small></span>
</details>